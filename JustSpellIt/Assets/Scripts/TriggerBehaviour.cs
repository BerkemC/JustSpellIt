...
public class TriggerBehaviour : MonoBehaviour {
	...
	void OnTriggerEnter2D (Collider2D other)
	{
		if(other.CompareTag("Teleport"))
		{
			...
			isTutorial = false;
			if(GameObject.Find("Canvas/InfoPanel"))GameObject.Find("Canvas/InfoPanel").SetActive(false);
			Destroy (GameObject.Find ("InfoPanel"));
			Destroy (GameObject.Find ("Info"));
			Destroy (GameObject.Find ("ButtonInfoStart"));
			Destroy (GameObject.Find ("ButtonInfoUndo"));
		...
		if(other.CompareTag("RockTriggerLeft")){
			GameObject.Find ("Traps/Rock").GetComponent<Rigidbody2D> ().isKinematic = false;
			GameObject.Find ("Traps/Rock").GetComponent<Rigidbody2D> ().velocity = new Vector3 (-1f, 0f, 0f);
			Destroy (other.gameObject);
		}
		if(other.CompareTag("RockTriggerRight")){
			GameObject.Find ("Traps/Rock").GetComponent<Rigidbody2D> ().isKinematic = false;
			GameObject.Find ("Traps/Rock").GetComponent<Rigidbody2D> ().velocity = new Vector3 (1f, 0f, 0f);
			Destroy (other.gameObject);
		}
